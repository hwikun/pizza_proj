import "components/PizzaModal/PizzaModal.css";
...
function Accordion({ content }) {
  ...
  return (
    <div className="Accordion" onClick={handleClick}>
      ...
      {!isHide && <div className="content">{content}</div>}
      ...
      {!isHide && (
        <div className="content">
          ...
          <p>
            도미노피자만을 위해 연구 개발된 도미노 시그니처 치즈는 미국
            농무성에서 인정한 A등급 프리미엄 우유로 생산되어 최고의 맛과 풍미를
            자랑하는 모차렐라 피자 치즈 입니다. 지방 함량과 단백질, 염도 함량을
            최적의 밸런스로 맞춰 고소한 맛과 치즈 풍미를 강화, 부드럽고 쫄깃하게
            늘어나는 치즈 스트레칭(늘어지는 정도)까지 고려한 최상의 치즈! 게다가
            페터 크림 치즈, 체더 치즈, 콰트로 치즈 퐁듀, 리코타 치즈 등 고급
            프리미엄 피자 치즈를 사용함으로써 피자의 품격을 높여 더욱 맛있는
            피자를 완성하였습니다.
          </p>
...
export default function PizzaModal({ closeModal, pizzaId }) {
  const [pizza, setPizza] = useState({});
  useEffect(() => {
    (async () => {
      const data = await (
        await fetch(`http://localhost:8082/api/pizza/${pizzaId}`)
      ).json();
      setPizza(data);
    })();
  });
    ...
    <div className="PizzaModal" onClick={closeModal}>
      <div className="panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="content">
          ...
          <Accordion content={pizza?.content} />
        </div>
      </div>
    </div>
  );
